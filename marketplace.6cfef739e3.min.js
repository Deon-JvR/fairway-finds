...
function optimizedListingImageUrl(value, width) {
const imageUrl = safeImageUrl(value);
if (!imageUrl) return "";
try {
const url = new URL(imageUrl, window.location.origin);
if (url.hostname !== "pyisbtqrwzqxlteibdqp.supabase.co") return imageUrl;
const params = new URLSearchParams({ url: imageUrl, w: String(width), q: "78", fit: "cover" });
return `/.netlify/images?${params.toString()}`;
} catch (error) {
return imageUrl;
}
}
function listingImageSrcset(value) {
return [320, 640, 960, 1280]
.map((width) => `${optimizedListingImageUrl(value, width)} ${width}w`)
.join(", ");
}
...
return `<img class="${className}" src="${escapeHtml(optimizedListingImageUrl(imageUrl, 800))}" srcset="${escapeHtml(listingImageSrcset(imageUrl))}" alt="${escapeHtml(alt)}" width="800" height="600" loading="${loading}" decoding="async" fetchpriority="${options.highPriority ? "high" : "low"}" sizes="${escapeHtml(sizes)}" onerror="this.hidden=true; this.closest('.photo-art')?.classList.add('image-load-failed');" />`;
}
...
<span>${escapeHtml(sellerDisplayLocation(seller, listing.location))}</span>
</div>
...
/\.(jpe?g|jfif|png|gif|webp|heic|heif|avif|bmp|tiff?)$/i.test(name);
}
...
urls.push(await uploadListingImage(supabase, user, file));
}
...
${imageUrl ? renderListingImage(imageUrl, "item-photo", listing.title) : renderGolfPlaceholder(`${listing.title} placeholder image`)}
</a>
...
const sellerLocation = sellerDisplayLocation(seller, data.location);
...
${thumbImage ? renderListingImage(thumbImage, "seller-thumb-photo", listing.title) : renderGolfPlaceholder(`${listing.title} placeholder image`)}
</a>
...
<label><span>Replace photos</span><input name="image_files" type="file" accept="image/*" multiple /></label>
</div>
...
setMarketplaceStatus("Listing saved and sent back to admin review.", "success");
setImageUploadStatus("");
await loadSellerListings();
await loadDashboardMetrics();
} catch (error) {
