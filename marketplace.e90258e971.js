...
function showMarketplaceToast(message, type = "success") {
  let region = document.querySelector("[data-marketplace-toasts]");
  if (!region) {
    region = document.createElement("div");
    region.className = "marketplace-toasts";
    region.dataset.marketplaceToasts = "";
...
function updateUploadProgress(statusElement, completed, total, state = "active") {
  if (!statusElement || !total) return;
  let wrapper = statusElement.parentElement?.querySelector("[data-upload-progress]");
  if (!wrapper) {
    wrapper = document.createElement("div");
    wrapper.className = "upload-progress";
    wrapper.dataset.uploadProgress = "";
    wrapper.innerHTML = '<progress max="1" value="0"></progress><span></span>';
  ...
  label.textContent = state === "error"
    ? `Upload stopped after ${completed} of ${total} photos.`
    : `${completed} of ${total} photos uploaded`;
}

function clearUploadRetry(statusElement) {
  statusElement?.parentElement?.querySelector("[data-upload-retry]")?.remove();
}

function showUploadRetry(form, statusElement) {
  if (!form || !statusElement) return;
  clearUploadRetry(statusElement);
  const retry = document.createElement("button");
  retry.type = "button";
  retry.className = "upload-retry-button";
  ...
  statusElement.insertAdjacentElement("afterend", retry);
}

function logUploadFailure(error, details = {}) {
  console.error("[Fairway Finds] Listing photo upload failed", {
    stage: details.stage || "upload",
    photoNumber: details.photoNumber,
    totalPhotos: details.totalPhotos,
...
async function prepareImageForUpload(file, reportStatus = setImageUploadStatus) {
  let source = file;
  if (isHeicFile(file)) {
    reportStatus("Converting iPhone photo for web display...", "info");
    await loadHeicConverter();
    const converted = await window.heic2any({ blob: file, toType: "image/jpeg", quality: 0.88 });
    source = Array.isArray(converted) ? converted[0] : converted;
  }

  reportStatus("Optimising photo size and quality...", "info");
  let bitmap;
  try {
...
async function uploadListingImage(supabase, user, file, options = {}) {
  const reportStatus = options.reportStatus || setImageUploadStatus;
  ...
  const uploadFile = await withTimeout(
    prepareImageForUpload(file, reportStatus),
    25000,
  ...
  reportStatus("Uploading image. This can take a few seconds...", "info");
  const { error } = await withTimeout(
    ...
    30000,
    "Upload stopped after 30 seconds. Check your connection and retry with a smaller image."
  );

  if (error) throw error;

  const { data } = supabase.storage.from("listing-images").getPublicUrl(path);
  reportStatus("Image uploaded.", "success");
  return { url: data.publicUrl, path };
}

async function uploadListingImages(supabase, user, fileList, options = {}) {
  const files = validateListingImageFiles(fileList);
  if (!files.length) return [];
  const statusElement = options.statusElement || imageUploadStatus;
  const reportStatus = (message, type = "info") => {
  ...
  try {
    for (const [index, file] of files.entries()) {
      reportStatus(`Uploading photo ${index + 1} of ${files.length}...`, "info");
      updateUploadProgress(statusElement, index, files.length);
      try {
        uploads.push(await uploadListingImage(supabase, user, file, { reportStatus }));
      } catch (error) {
        logUploadFailure(error, { stage: "prepare-or-upload", photoNumber: index + 1, totalPhotos: files.length, file });
        updateUploadProgress(statusElement, index, files.length, "error");
        throw error;
      }
      updateUploadProgress(statusElement, index + 1, files.length, index + 1 === files.length ? "success" : "active");
    }
  } catch (error) {
  ...
  reportStatus(`${urls.length} photo${urls.length === 1 ? "" : "s"} uploaded.`, "success");
  return urls;
}
...
if (listingForm) {
  ...
  listingForm.addEventListener("submit", async (event) => {
    ...
    clearUploadRetry(imageUploadStatus);

    try {
      const imageUrls = await uploadListingImages(supabase, user, listingForm.image_files?.files, { statusElement: imageUploadStatus });
      ...
      showMarketplaceToast("Listing saved and submitted for review.", "success");
      ...
      const message = error.message || "Could not save this listing.";
      if (/image|photo|upload|timed out|seconds/i.test(message)) {
        setImageUploadStatus(message, "error");
        showUploadRetry(listingForm, imageUploadStatus);
      }
      setMarketplaceStatus(message, "error");
      showMarketplaceToast(message, "error");
    } finally {
...
if (sellerListings) {
  ...
  sellerListings.addEventListener("submit", async (event) => {
    ...
    clearUploadRetry(editImageStatus);
    ...
    try {
      ...
      if (imageFiles && imageFiles.length) {
        ...
        const imageUrls = await uploadListingImages(supabase, user, imageFiles, { statusElement: editImageStatus });
      ...
      showMarketplaceToast("Listing saved successfully.", "success");
      setImageUploadStatus("");
      ...
      showMarketplaceToast(message, "error");
      if (/image|photo|upload|timed out|seconds/i.test(message)) {
        showUploadRetry(form, editImageStatus);
      }
    } finally {
